...
namespace memory {
	...
	inline uintptr_t FindPatternW(uintptr_t start, size_t length, const unsigned char* pattern, const char* mask)
	{
		size_t pos = 0;
	...
	inline uintptr_t FindPattern(HMODULE module, const unsigned char* pattern, const char* mask)
	{
		...
		return FindPatternW(reinterpret_cast<uintptr_t>(module), info.SizeOfImage, pattern, mask);
	}
